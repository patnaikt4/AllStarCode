...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  // Helper: mark row as failed and return a consistent error response.
  // Defined as a named async function (not arrow) to avoid closure issues with ESLint.
  async function failWith(code: string, message: string, httpStatus: number): Promise<NextResponse> {
    const safeMessage = message.slice(0, 500) // never store arbitrary LLM/API text verbatim
  ...
  if (updateError) {
    // PDF uploaded but DB update failed — mark failed so user can retry (upload will upsert).
    return failWith(
      'DB_ERROR',
